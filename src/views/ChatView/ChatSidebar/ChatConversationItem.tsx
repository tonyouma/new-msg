...
const getDetails = (conversation: Conversation, currentUserId: string) => {
  ...
  const displayNames = groupParticipants
...
const ChatConversationItem: React.FC<ChatConversationItemProps> = (props) => {
  const { isSelected, conversation, onSelectConversation, isOpenSidebar } =
    props;
  ...
  const isOnlineGroup =
    groupChat &&
    details.groupParticipants.map((item) => item.status).includes("online");
